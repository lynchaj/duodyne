...
get_frame_segment:
%if 0		; obsolete
; compute the frame segment from the 18h (Tiny Basic) interrupt vector
	pushm	ds
	push	0
	pop	ds
...
%else
	...
	mov	dx,cpu_mmcs	; find memory hole
	in	ax,dx		;
	and	ax,0FE00h
	cmp	ax,7000h	; was 8000h, but disabled EXT_MEM
				;  will map to 7000h area
	jb	.5		; version 1 board if below
	; the Middle memory is part of the low 512K memory select
; version 3 board
	mov	bx,ax		; save start of EXT_MEM in BX
	mov	dx,cpu_mpcs	; find size of EXT_MEM
	in	ax,dx		;
	shl	ax,1
	and	ax,0FE00h	;
	xchg	bx,ax		;

	cmp	bh,10h		; compare to 64k
   	jae	.9		; 64K or more; use lowest 64k
; must have 64K
	popm	bx,dx
	jmp	installation_error	; (I hope ES is intact)
;
;
; version 1 board
; get segment 64K below the start of ROM
;
.5:
	mov	dx,cpu_umcs	; get upper memory start address
	in	ax,dx		; from the upper memory select
	xor	al,al		; get segment
	sub	ah,10h	 	; deduct 64K
.9:
	popm	bx,dx
%endif
	mov	word [frame],ax
