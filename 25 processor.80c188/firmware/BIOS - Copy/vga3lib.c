...
#if WAIT_HOR
/* nothing gets this to work so far */
void wait_horizontal_retrace(void)
{
	while ((rd_crtc(31) & 0x80) != 0) ;	/* wait for end of horizontal retrace */
	while ((rd_crtc(31) & 0x80) == 0) ; 	/* wait for start of horizontal retrace */
}
#endif
